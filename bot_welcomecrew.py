...
class YesNoSelect(discord.ui.Select):
    ...
    async def callback(self, itx: discord.Interaction):
        ...
        if self.key == "siege_interest":
            self.sess.have_siege = True
        if self.key == "cvc_interest":
            self.sess.have_cvc_interest = True
...
async def on_thread_create(thread: discord.Thread):
    if not thread.parent or thread.parent.id != WELCOME_CHANNEL_ID:
        return
    if not re.match(THREAD_NAME_REGEX, thread.name or ""):
        return
...
async def on_message(message: discord.Message):
    if not isinstance(message.channel, discord.Thread):
        return
    if TICKET_TOOL_BOT_ID and message.author.id != TICKET_TOOL_BOT_ID:
        return
    ...
    if "ticket closed" not in merged:
        return
    ...
    if not re.match(r"(?i)closed-", message.channel.name or ""):
        return
...
async def _find_closed_timestamp(thread: discord.Thread) -> str:
    ...
    try:
        ...
        if last:
            return last[0].created_at.strftime("%Y-%m-%d %H:%M")
    except Exception:
        pass
...
# ---------- Render web service runner (keeps free plan alive) ----------
